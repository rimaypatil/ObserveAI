...
async def create_api_key(
    ...
    """Generates a new SDK API key for high-throughput ingestion (Owner only)."""
    api_key, raw_key = await project_service.create_api_key(session, project_id, current_owner.organization_id, data)
...
async def delete_api_key(
    ...
    """Deletes an API key (Owner only)."""
    await project_service.delete_api_key(session, key_id, current_owner.organization_id)
    return APIResponse(
